        ...
        for journalURL in journalURLs {
            ...
            guard DocumentIdentity.make(url: targetURL).stableKey
                  ...
                  candidateURL.deletingLastPathComponent().standardizedFileURL
                    == replacementDirectoryURL.standardizedFileURL else {
                throw JournalError.invalidArtifactPath
            }
            if terminalState == .prepared {
                guard let directoryIdentifier =
                        try? DurableFileIO.resourceIdentifier(
                            for: replacementDirectoryURL
                        ),
                      directoryIdentifier
            ...
            let swapCompleted = terminalState == .prepared
                && (
                    targetIdentifier
                        == journal.preparedCandidateResourceIdentifier
                    || (
    ...
    static func reconcileCommit(
        ...
        guard request.token.operation == .commitReconciliation,
              ...
              request.pendingSave.expectedDurableState
                == expectedBaseline.asDurableFileState else {
            return .unresolved
        }
        ...
        let matching = try pendingRecoveries(in: recoveryDirectory).filter {
            guard $0.documentIdentity == request.identity,
                  ...
                  let binding = $0.artifact.binding else {
                return false
            }
        ...
        case .committed:
            guard targetFingerprint.resourceIdentifier
                  ...
                  targetFingerprint.contentDigest
                    == request.pendingSave.contentFingerprint.contentDigest else {
                return .unresolved
            }
            try acknowledge(
                pending.artifact,
        ...
        case .preparedCandidateRemains:
            ...
            guard candidateIdentifier
                    == pending.preparedCandidateResourceIdentifier,
                  targetFingerprint == expectedBaseline.fingerprint else {
                return .unresolved
            }
            try acknowledge(
                pending.artifact,
