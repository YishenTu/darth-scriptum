...
extension DocumentSyncReducer {
    ...
    static func externalReadFinished(
        ...
        case .unchanged(let observation):
            guard let expectedBaseline = attempt.expectedBaseline,
                  ...
                  observation.fingerprint == expectedBaseline.fingerprint else {
    ...
    static func mergeFinished(
        ...
        guard accepts(state, token: token),
              ...
              merge.token == token,
              result.token == token,
              result.request == DocumentSyncMergeRequest(
                token: merge.token,
                base: merge.base,
                local: merge.local,
                external: merge.external.snapshot,
        ...
        guard updated.source == merge.localSourceRevision,
              ...
              updated.durableBaseline == merge.baseline else {
        ...
        case .conflict:
            if case .displacedPreimage(
                ...
                guard let effects = persistDisplacedPreimageConflict(
                    &updated,
                    merge: merge,
            ...
            guard let effects = persistLocalConflict(
                &updated,
    ...
    static func persistLocalConflict(
        ...
        guard let attachment = state.fileAttachment,
              case .ready = state.recoveryAccess else {
            return nil
        }
