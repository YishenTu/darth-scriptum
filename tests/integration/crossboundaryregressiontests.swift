...
final class CrossBoundaryRegressionTests: XCTestCase {
    ...
    func testManagedCloseWaitsForBlockedRecoveryStartup() async throws {
        ...
        guard case .closing(let attempt) =
            harness.coordinator.reducerState.lifecycle else {
        ...
        gate.release()
        guard case .ready =
            await harness.coordinator.waitForRecoveryStartup() else {
    ...
    func testQuitWithCleanFailingAndUntitledDocumentsPreservesRefusal()
        ...
        guard case .ready =
            await cleanDocument.syncCoordinator.waitForRecoveryStartup() else {
        ...
        guard case .ready =
            await refusalDocument.syncCoordinator.waitForRecoveryStartup() else {
...
private final class Q1ControllableEffectExecutor:
    ...
    private(set) var externalReadRequests:
    ...
    func finishSavePreparation(
        ...
        guard let completion =
            savePreparationCompletions.removeValue(forKey: token) else {
            return false
        }
        completion(result)
        return true
    ...
    func finishExternalRead(
        ...
        guard let completion =
            externalReadCompletions.removeValue(forKey: token) else {
            return false
        }
        completion(result)
        return true
    ...
    func finishMerge(
        ...
        guard let completion =
            mergeCompletions.removeValue(forKey: token) else {
            return false
        }
        completion(result)
        return true
