...
final class CrossBoundaryRegressionTests: XCTestCase {
    func testBlockedManagedSaveLeavesTheMainActorResponsive() async throws {
        ...
        let request = try makeQ1CommitRequest(targetURL: fixture.url)
        try document.syncCoordinator.bridge.install(request)
        let documentBox = Q1UncheckedDocumentBox(document)
        ...
        let mainActorResponded = expectation(
            description: "main actor remains responsive during blocked commit"
        )
        Task { @MainActor in
            order.append("main")
        ...
        XCTAssertEqual(order.values, ["unblock", "commit", "main"])
    ...
    func testDocumentFileAccessLanesPreserveFIFOWithoutCrossDocumentBlocking()
        async throws
    {
        let blockedLane = DocumentFileAccessLane(
            label: "test.integration.document-lane.blocked"
        )
        let independentLane = DocumentFileAccessLane(
            label: "test.integration.document-lane.independent"
        )
        let gate = Q1BlockingIOGate()
        let order = Q1IntegerRecorder()
        defer { gate.release() }
        ...
        let independentValue = try await independentLane.perform { 42 }
    ...
    func testConcurrentDocumentReadStagesOffMainBeforeMainActorInstallation()
        async throws
    {
        let document = MarkdownDocument()
        ...
        let readWasOnMainThread = try await Task.detached {
            let wasOnMainThread = q1IsMainThread()
        ...
        XCTAssertFalse(readWasOnMainThread)
        XCTAssertFalse(document.hasInitializedSynchronization)
        XCTAssertEqual(
...
private final class Q1StringRecorder: @unchecked Sendable {
    private let lock = NSLock()
...
private final class Q1IntegerRecorder: @unchecked Sendable {
    private let lock = NSLock()
...
private struct Q1BlockingDocumentFileCommitter: DocumentFileCommitting {
    let gate: Q1BlockingIOGate
    let order: Q1StringRecorder
...
private final class Q1UncheckedDocumentBox: @unchecked Sendable {
...
private func makeQ1CommitRequest(
    targetURL: URL
    ...
    let pendingSave = PendingSaveToken(
        ...
        preparedPayload: try TextFileCodec.prepareSavePayload(
            for: DocumentSnapshot(text: "saved\n", format: .newDocument)
        ),
...
private func q1IsMainThread() -> Bool {
    Thread.isMainThread
}
