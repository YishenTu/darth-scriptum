...
extension DocumentSyncReducerTests {
    ...
    func testLateMergeCannotOverwriteNewerLocalRevision() throws {
        ...
        XCTAssertEqual(newer.effects.first, .cancelOperation(merge.token))
    ...
    func testAtomicSwapUnavailableRequiresSaveAsWithoutRetryingCommit() throws {
        ...
        let durableSnapshot = try XCTUnwrap(
            write.initial.durableBaseline?.snapshot
        )
        let revertedRevision = SourceRevision(
            number: laterLocalRevision.number + 1,
            text: durableSnapshot.text
        )
        let reverted = DocumentSyncReducer.reduce(
            edited.state,
            event: .sourceChanged(
                revertedRevision,
                format: durableSnapshot.format
            )
        )

        XCTAssertEqual(reverted.state.local, .clean(revertedRevision))
        XCTAssertNil(reverted.state.issue)
        XCTAssertFalse(reverted.state.statusProjection.failureRequiresSaveAs)
    }
