...
final class AppDelegateTests: XCTestCase {
    func testFileMenuContainsStandardResponderChainSaveAndSaveAs() {
    ...
    func testEditMenuRoutesUndoAndRedoThroughTheResponderChain() {
        let originalMenu = NSApp.mainMenu
        defer {
            NSApp.mainMenu = originalMenu
        }
        ...
        XCTAssertEqual(
            undoItem?.action,
            #selector(MarkdownDocument.undoDocument(_:))
        )
    ...
    func testDocumentValidatesSaveUndoAndRedoFromOwnedState() async throws {
        ...
        saveItem.action = #selector(NSDocument.save(_:))
        let undoItem = NSMenuItem()
        undoItem.action = #selector(MarkdownDocument.undoDocument(_:))
        let redoItem = NSMenuItem()
        redoItem.action = #selector(MarkdownDocument.redoDocument(_:))
        let fileURL = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString + ".md")
        try Data().write(to: fileURL)
        defer { try? FileManager.default.removeItem(at: fileURL) }
        document.fileURL = fileURL
        let didAttach = await document.syncCoordinator.attachAndWait(
            to: fileURL,
            knownData: Data()
        )
        XCTAssertTrue(didAttach)

        XCTAssertFalse(document.validateUserInterfaceItem(saveItem))
        XCTAssertFalse(document.validateUserInterfaceItem(undoItem))
        XCTAssertFalse(document.validateUserInterfaceItem(redoItem))
        ...
        XCTAssertTrue(document.validateUserInterfaceItem(saveItem))
        XCTAssertTrue(document.validateUserInterfaceItem(undoItem))
        XCTAssertFalse(document.validateUserInterfaceItem(redoItem))
        document.undoDocument(nil)
        XCTAssertFalse(document.validateUserInterfaceItem(undoItem))
        XCTAssertTrue(document.validateUserInterfaceItem(redoItem))
        XCTAssertFalse(document.syncCoordinator.hasLocalChanges)
        XCTAssertFalse(document.validateUserInterfaceItem(saveItem))
    }
