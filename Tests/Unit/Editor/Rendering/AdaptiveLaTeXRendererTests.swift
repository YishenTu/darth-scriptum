...
final class AdaptiveLaTeXRendererTests: XCTestCase {
    ...
    func testPresentationChangePrioritizesCurrentFallbackWork() async throws {
        ...
        let theme = MarkdownEditorTheme()
        let staleFormulas = (0..<(AdaptiveLaTeXRenderer.maximumPendingEntries + 4)).map {
            "stale-\($0)"
        }
    ...
    func testSourceModeDiscardsQueuedFallbackWork() async throws {
        ...
        let theme = MarkdownEditorTheme()
        let formulas = (0..<(AdaptiveLaTeXRenderer.maximumPendingEntries + 4)).map {
            "formula-\($0)"
        }
...
private final class ControllableMathJaxFallback: MathJaxFallbackRendering {
    ...
    private var continuations: [String: CheckedContinuation<MathJaxFallbackRenderOutcome, Never>] =
        [:]
...
extension MathJaxFallbackRenderOutcome {
    fileprivate var renderedResult: LatexRenderResult? {
        if case .rendered(let output) = self {
