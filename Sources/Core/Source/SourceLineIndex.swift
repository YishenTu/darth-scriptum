    ...
    private mutating func makeTree(
        ...
        while location < limit {
            ...
            if unit == 0x000D {
                ...
                if location <= limit {
                    guard
                        (tree?.entryCount ?? 0) + block.count
                            < Self.maximumStoredLineStarts
                    else {
                        return nil
                    }
...
private struct SampledSourceLineIndex: Sendable {
    ...
    private init?(cancellableText text: String) {
        ...
        textLength = source.length
        guard
            let checkpoints = Self.scanCheckpoints(
                in: source,
                from: SourceLineCheckpoint(
                    offset: 0,
                    line: 1,
    ...
    mutating func apply(
        ...
        guard
            let rescanned = Self.scanCheckpoints(
                in: updated,
                from: rescanStart,
                through: newBoundaryOffset,
                cancellationChecksEnabled: false
            ), let newBoundary = rescanned.last
        else {
            return false
        }
